...
%--------------------------------------------------------------------------
%   计算解码过程中所用的反方向状态转移图:
%   last_state_smaller(state_index+1)      ,  表示可迁移到state_index状态的较小的状态——（0—31）
...
%  因为是从中途开始回溯，所以先找到最小度量对应的状态，然后从该状态开始回溯　
%  path_state_min用来存放回溯过程中经历的状态

[metric_min,from_state_plus1] = min(state_metrics); 
from_state = from_state_plus1 -1; % 找到最小度量对应的状态编号。
path_state_min(decode_steps) = from_state; 

for step_index = decode_steps : -1 : 1
    branch_record = path_record(from_state+1, step_index);
    path_state_min(step_index) = from_state;
    if branch_record == 0
...
%  从所有64个状态开始回溯，回溯结果用于和从最小状态开始的回溯结果相比较
%  path_state_all用来存放回溯过程中经历的状态　

for state_index = 0 : states_num-1
    path_state_all(decode_steps, state_index+1) = state_index;
    from_state = state_index;
    for step_index = decode_steps : -1 : 1
        path_state_all(step_index, state_index+1) = from_state;
        branch_record = path_record(from_state+1, step_index);
        if branch_record == 0
            from_state = last_state_smaller(from_state+1);
        else
...
%  为每个译码步骤的每个状态画出一个点
for step_index = 1 : 1 : decode_steps
    for state_index = 0 : 1 : states_num-1
        a_dot_for_a_state(state_index+1,step_index) = state_index;
    end
end   
x = 1: decode_steps;
plot(x, a_dot_for_a_state, '.k');
hold on;

%  从64个状态开始回溯得到的路径，蓝色
plot(x, path_state_all, '-b');
hold on;

%  从最小状态开始回溯得到的路径，红色
plot(x, path_state_min, '-r');
