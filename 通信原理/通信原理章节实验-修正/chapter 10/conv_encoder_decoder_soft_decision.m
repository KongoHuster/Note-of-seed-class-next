...
decoder_input = noise_added;%  解码输入
...
for step_index = 1 : decode_steps 
    ...
    if step_index <= coder_constraint-1  %  因为初始状态已知为全零状态，所以做特殊处理
        ...
        for state_index = 0 : pow2(step_index)-1
            branch_metrics_from_smaller(state_index+1) = abs(decoder_input(2 * step_index - 1) - (1-2*outbit1_from_last_state_smaller(state_index+1))).^2 + abs(decoder_input(2 * step_index) - (1-2*outbit2_from_last_state_smaller(state_index+1))).^2;
        ...
        for state_index = 0 : states_num-1
            branch_metrics_from_smaller(state_index+1) = abs(decoder_input(2 * step_index - 1) - (1-2*outbit1_from_last_state_smaller(state_index+1))).^2 + abs(decoder_input(2 * step_index) - (1-2*outbit2_from_last_state_smaller(state_index+1))).^2;
            branch_metrics_from_larger(state_index+1) = abs(decoder_input(2 * step_index - 1) - (1-2*outbit1_from_last_state_larger(state_index+1))).^2 + abs(decoder_input(2 * step_index) - (1-2*outbit2_from_last_state_larger(state_index+1))).^2;
            path_metrics_from_smaller(state_index+1) = state_metrics_last(last_state_smaller(state_index+1) + 1) + branch_metrics_from_smaller(state_index+1);
