...
void DisplayDemoPrintMenu(DisplayCtrl *dispPtr)
{
	...
	xil_printf("5 - Print Test 1\n\r");
	xil_printf("6 - Print Test 2\n\r");
	xil_printf("7 - Invert Current Frame colors\n\r");
	xil_printf("8 - Invert Current Frame colors seamlessly*\n\r");
	xil_printf("q - Quit\n\r");
	xil_printf("\n\r");
...
unsigned char Mandelbrot(int i,int j){
...
unsigned short swirlyRD(int i,int j){
	#define D(x) (x-DIM/2.)/(DIM/2.)
	float x=D(i),y=D(j),X,Y,n=0;while(n++<200&&(X=x*x)+(Y=y*y)<6){x=X-Y+.3627;y=2*x*y+.32;}return log(n)*256;
}
unsigned short swirlyGR(int j,int i){
	//float x=D(i),y=D(j),X,Y,n=0;while(n++<200&&(x*x+y*y)<4){X=x;Y=y;x=X*X-Y*Y+-.7;y=2*X*Y+.27015;}return log(n)*128;
	float x=D(i),y=D(j),X,Y,n=0;while(n++<100&&(x*x+y*y)<4){X=x;Y=y;x=X-X*Y+-.27;y=2*X*Y+.21015;}return log(n)*64/2;
}
unsigned short swirlyBL(int i,int j){
	float x=D(i),y=D(j),X,Y,n=0;while(n++<600&&(x*x+y*y)<4){X=x;Y=y;x=X*X-Y*Y+.36237;y=2*X*Y+.32;}return log(n)*128;
//	float x=D(i),y=D(j),X,Y,n=0;while(n++<30&&(x*x+y*y)<4){X=x;Y=y;x=X*Y-Y+.3137;y=2*X*Y+.32;}return log(n)*128;
}
...
void DisplayDemoPrintTest(u32 *frame, u32 width, u32 height, u32 stride, int pattern)
{
	...
	case DISPLAYDEMO_PATTERN_3:
		wStride = stride/4;
		for(xcoi = 0; xcoi < width; xcoi++) {
			for(ycoi = 0; ycoi < height; ycoi++) {
				iPixelAddr = xcoi + ycoi*wStride;

//				fRed = swirlyRD(ycoi, xcoi);
//				fBlue = swirlyBL(xcoi, ycoi);
//				fGreen = swirlyGR(xcoi, ycoi);
